...
async function fetchInterpellations(legislatorName: string): Promise<Interpellation[]> {
    ...
    const apiUrl = `https://ly.govapi.tw/v2/interpellations?filter=${encodeURIComponent('屆')}=11&filter=${encodeURIComponent('質詢委員')}=${encodedName}`
...
async function processInterpellations(
    ...
    for (const [weekKey, weekInterpellations] of interpellationsByWeek.entries()) {
        ...
        // Create a detailed reason with 事由 and 說明
        let reason = `Spoke on ${count} topic${count > 1 ? 's' : ''} on Legislative Yuan floor`
