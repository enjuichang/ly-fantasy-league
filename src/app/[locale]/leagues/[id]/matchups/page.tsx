...
export default async function MatchupsPage({ params, searchParams }: MatchupPageProps) {
    ...
    const t = await getTranslations('matchupsPage')
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        {t('week', { week: selectedWeek, total: league.totalWeeks })}
                        {selectedWeek === currentWeek && ` • ${t('currentWeek')}`}
                    </p>
                </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
                {Array.from({ length: league.totalWeeks }, (_, i) => i + 1).map((week) => (
                    ...
                    >
                        {t('week', { week, total: league.totalWeeks }).split(' / ')[0]}
                        {week === currentWeek && ' •'}
                    </a>
                ))}
            </div>
            ...
            {matchups.length === 0 ? (
                <Card>
                    <CardContent className="py-8 text-center text-muted-foreground">
                        <p>{t('noMatchups.title')}</p>
                        <p className="text-sm mt-2">
                            {t('noMatchups.description')}
                        </p>
                    </CardContent>
                </Card>
            ) : (
                        ...
                        return (
                            <Card key={matchup.id} className="overflow-hidden">
                                <CardContent className="p-0">
                                        ...
                                        <div className="flex flex-col items-start">
                                            {isBye ? (
                                                <div className="text-center w-full pt-4">
                                                    <p className="text-muted-foreground italic">{t('byeWeek')}</p>
                                                </div>
                                            ) : matchup.team2 ? (
                                                <>
                                    ...
                                    {/* Week info footer */}
                                    <div className="bg-muted px-6 py-2 text-xs text-muted-foreground border-t">
                                        {t('week', { week: matchup.week, total: league.totalWeeks })} • {new Date(matchup.weekStart).toLocaleDateString()}
                                        {hasScores && <span> • {t('scoresCalculated')}</span>}
                                    </div>
                                </CardContent>
                            </Card>
                        )
